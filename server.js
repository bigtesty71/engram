...
// ── Boot status for diagnostics ──
let bootStatus = 'starting';
let bootError = null;

// ── Diagnostic ping (registered before anything else) ──
app.get('/ping', (req, res) => {
    res.json({ status: bootStatus, error: bootError, node: process.version, uptime: process.uptime() });
});

// ── START SERVER IMMEDIATELY ──
// This must happen BEFORE routes/api is required,
// so Passenger sees the app alive even if a dependency fails
app.listen(PORT, () => {
